...
public class DungeonGenerator : MonoBehaviour
{
    ...
    public void SetupScene(int level)
    {
        ...
        DungeonMapTexture = transform.Find("DungeonMiniMapRepresentation").gameObject;
        terrainDungeonMap = GameObject.Find("Terrain").GetComponent<Terrain>();
    ...
    void RoomGenerator_OnRoomsGenerated()
    {
        ...
        // Generate the terrain based on the populated Grid object
        terrainDungeonMap.terrainData = GenerateTerrain(terrainDungeonMap.terrainData);

        RoomGenerator.ClearInvisibleData();
    }
    ...
    TerrainData GenerateTerrain(TerrainData td)
    {
        ...
        int higher = width > height ? width : height;
        td.heightmapResolution = Grid.GetLength(0) + 1;
        td.size = new Vector3(higher, 45, higher);

        td.SetHeights(0, 0, GenerateHeightsFromGrid(Grid.GetLength(0), Grid.GetLength(1)));
        return td;
    }

    float[,] GenerateHeightsFromGrid(int width, int height)
    {
        int higher = width > height ? width : height;
        float[,] heights = new float[higher, higher];

        for (var x = 0; x < width; x++)
        {
            for (var y = 0; y < height; y++)
            {
                if (x > higher || y > higher)
                {
                    heights[x, y] = 1000f;
                }
                else if (Grid[x, y] == 0)
    ...
    /// <summary>
    /// Utility function to emplace a door at the entrance of a new room
    /// </summary>
    /// <param name="p0">starting point of door</param>
    /// <param name="p1">ending point of door</param>
    ...
    /// <summary>
    /// Offload complexity function to create a 2D Texture by specifying different colors as different room types
    /// </summary>
